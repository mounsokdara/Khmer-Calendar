  ...
  Widget _taskGroup(BuildContext context, String title, List<CalendarEvent> list, bool overdue) {
    ...
    return Column(
      ...
      children: [
        Padding(
          padding: const EdgeInsets.fromLTRB(16, 12, 16, 4),
          child: Text(title, style: Theme.of(context).textTheme.titleSmall?.copyWith(color: overdue ? Colors.red : null)),
        ),
        for (final e in list)
          ListTile(
            leading: Checkbox(value: e.done ?? false, onChanged: (_) => store.toggleEventDone(e.id)),
            title: Text(e.title, style: TextStyle(decoration: e.done == true ? TextDecoration.lineThrough : null)),
