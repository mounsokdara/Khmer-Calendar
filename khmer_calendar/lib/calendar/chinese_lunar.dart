...
int _word(int year) => _lunarWords[year - 1900];

int _monthDays(int year, int month) => (_word(year) & (0x10000 >> month)) != 0 ? 30 : 29;

int _leapMonth(int year) => _word(year) & 0xf;

int _leapDays(int year) {
  if (_leapMonth(year) == 0) return 0;
  return (_word(year) & 0x10000) != 0 ? 30 : 29;
}

int _yearDays(int year) {
  var sum = 348;
  for (var i = 0x8000; i > 0x8; i >>= 1) {
    if ((_word(year) & i) != 0) sum += 1;
  }
...
ChineseLunar? solarToChineseLunar(int year, int month, int day) {
  ...
  final months = <({int month, int days, bool leap})>[];
  for (var m = 1; m <= 12; m += 1) {
    months.add((month: m, days: _monthDays(lunarYear, m), leap: false));
    if (m == leap) {
      months.add((month: m, days: _leapDays(lunarYear), leap: true));
    }
  }

  var lunarMonth = 1;
  var isLeap = false;
  for (final slot in months) {
    if (offset < slot.days) {
  ...
  return ChineseLunar(year: lunarYear, month: lunarMonth, day: offset + 1, isLeap: isLeap);
}
